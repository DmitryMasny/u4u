...
import { modalUploadStickersSelector } from "__TS/selectors/modals"
...
interface ImodalUploadStickers {
    ...
    // moveStickerAction: (name: string)=>any;     // экшен перенести стикер
}
interface IstickersUploadProgressBar {
    uploadCount?: number;
    totalPercent?: number;
    errorCount?: number;
}
...
const FILE_MAX_WEIGHT = 15 * 1024 * 1024; // Максимальный размер файла
...
/**
 *  Прогресс загрузки стикеров
 */
const StickersUploadProgressBar: React.FC<IstickersUploadProgressBar> = ({ uploadCount, totalPercent, errorCount }) =>
    ...
    const localStickerConfig = {    // Саша, прости :)
        ext: "png",
        md: "md",
        ms: "ms",
        orig: "orig",
 ...
 */
const ModalUploadStickers: React.FC<ImodalUploadStickers> = () => {
    const modalData = useSelector( modalUploadStickersSelector );
    ...
    useEffect(()=>{
        setModal({
            footer: [
                ...
                uploadInfo.totalCount ? {type: TYPES.COMPONENT, component:  <StickersUploadProgressBar {...uploadInfo}/>} : {type: TYPES.DIVIDER},
                {type: TYPES.BTN, text: 'Готово', intent: uploadInfo.totalPercent >= 100 ? 'primary' : '', action: completeUploadHandler, disabled: uploadInfo.disabled },
            ]
        });
    }, [uploadInfo.totalPercent, uploadInfo.disabled, uploadList]);
        ...
        if ( uploadList.length && uploadList.length > uploadInfo.totalCount || uploadInfo.totalPercent < 100 ) {
            ...
            const uploadCallback = ({data, error}) => {
                setUploadList(uploadList.map((item, i)=>
                    i === nextUpload.index ?
                        (error || !data ? {...item, error: error } : {...data, file: item.file, name:  item.file.name, src: createStickerLink({stickerConfig: localStickerConfig, id: data.id, size: 'sm'}) })
                        : item));
            };
            ...
            if (completeList.length || errorList.length) {
                const totalPercent = Math.round((completeList.length + errorList.length)/ uploadList.length * 100);
                setUploadInfo({
                    ...
                    disabled: uploadList.length && totalPercent && totalPercent < 100
                });
    ...
    // }, [ uploadList, ...(uploadInfo ? [uploadInfo.totalCount, uploadInfo.alreadyCount, uploadInfo.totalPercent] : []) ]);
    }, [ uploadList ]);
...
export default memo(ModalUploadStickers);
