...
import { windowIsMobileSelector } from '__TS/components/Editor/_selectors'
...
const geoJson = {
    ...
    "features": [{
        ...
        "properties": {
            "description": 'Доставка за пределами МКАД осуществяется сторонними компаниями в разделе "Доставка по России"',
            "fill": "#990000",
            "fill-opacity": 0,
            "stroke": "#990000",
            "stroke-width": "0",
        ...
        "geometry": {
            ...
            "coordinates": [
                [
                    [37.553308915039, 55.77538176142222], [37.54232258691399, 55.77189843089684], [37.534082840820254, 55.76376944383762], [37.532022904296824, 55.74828094941678], [37.53476948632807, 55.739372267918796], [37.54300923242183, 55.73356115773891], [37.550562333007754, 55.72619917080602], [37.557428788085865, 55.71961094803531], [37.57322163476555, 55.71728542642805], [37.585581253906184, 55.70836963868542], [37.59794087304682, 55.70216614626338], [37.613733719726504, 55.70100288132658], [37.623346756835886, 55.70488029589374], [37.63570637597649, 55.706043444927204], [37.652872513671824, 55.70255389351524], [37.66179890527337, 55.7029416369036], [37.67072529687495, 55.71147101402576], [37.685831498046824, 55.71534738553147], [37.700251053710886, 55.71883578961672], [37.714670609374956, 55.724649101163536], [37.699564408203074, 55.736273117282344], [37.69819111718743, 55.74518250926443], [37.697504471679615, 55.751766399222355], [37.69338459863275, 55.75951072362325], [37.692697953124934, 55.76841478773547], [37.685831498046824, 55.77422068598347], [37.67553181542963, 55.78002571582685], [37.66591877832024, 55.78466911449842], [37.65699238671869, 55.79202002593352], [37.64806599511712, 55.795501550420965], [37.62815327539057, 55.79318056882262], [37.60480732812493, 55.793567408735214], [37.58832783593743, 55.79318056882262], [37.57802815332026, 55.79240687742178], [37.567041825195254, 55.787377507097496], [37.56086201562495, 55.78079965419703], [37.553308915039, 55.77538176142222]
                ].map( ( item ) => [item[ 1 ], item[ 0 ]] )
            ]
        },
        "properties": {
            "description": "Доставка в пределах ТТК: 450 руб.",
            "fill": "#65c334",
            "fill-opacity": 0.4,
            "stroke": "#65c334",
            "stroke-width": "5",
...
const BadZoneContent = ReactDOMServer.renderToStaticMarkup (<AddressText>
    <span className={"danger"}>По указанному адресу доставка осуществляется транспортной компанией. В разделе "Доставка по России"</span>
</AddressText>);

const AddressBlock = ({adr={}, goToOtherDelivery }) => {
    if (adr.notAvaible || !adr.zoneId) {
        return adr.address && adr.building && adr.zoneId &&
            <AddressText>
            <span className={"danger"}>Доставка за пределами МКАД осуществляется транспортной компанией. В разделе&nbsp;
                <a onClick={goToOtherDelivery}>"Доставка по России"</a></span>
            </AddressText>
            ||
            <AddressText>
                <span className={(adr.address && adr.notAvaible) ? "danger" : ""}>Выберите адрес в пределах МКАД</span>
            </AddressText>
    } else {
        return adr.address && adr.building &&
            <AddressText>
                Адрес доставки: <span className={"address"}>{adr.address}, {adr.building}</span>.
                {adr.price && <span className="priceSpan">Стоимость доставки: <span className={"price"}>{adr.price}руб.</span></span>}
            </AddressText>
            ||
            <AddressText>
                { adr.building ? <span className={"danger"}>Выберите корректный адрес</span> : <span className={"danger"}>Укажите номер дома</span> }
            </AddressText>
    }
};

const U4UDelivery = ({changeDeliveryData, addressObj={}, errors={} }) => {
    const [ loaded, setLoaded ] = useState( false );
    const isMobile = useSelector( windowIsMobileSelector );

    const init = () => {
        const myMap = new ymaps.Map('map', {
            center: [55.733, 37.638],
            zoom: 10,
        ...
        const onZonesLoad = ( json ) => {
            ...
            // Выбор адреса в выпадающем списке
            ...
            // Геокодирование после выбора адреса в выпадающем списке
            ...
            function highlightResult(obj) {
                ...
                const coords = obj.geometry.getCoordinates(),
                    bounds = obj.properties.get('boundedBy'),   // Область видимости геообъекта.
                    containingPolygons = deliveryZones.searchContaining(coords);
                const containingPolygonsLength = containingPolygons.getLength();
                // Находим полигон, в который входят переданные координаты.
                const polygon = deliveryZones.searchContaining(coords).get(containingPolygonsLength - 1);
                let notAvaible = false;
                ...
                if ( polygon && polygon.options._options.fillColor !== "#990000"  ) {
                    // Перемещаем метку с подписью в переданные координаты и перекрашиваем её в цвет полигона.
                    deliveryPoint.properties.set( {
                        iconCaption: 'Доставка курьером U4U',
                        balloonContentHeader: 'Доставка курьером U4U',
                        // balloonContentHeader: obj.getAddressLine(),
                        balloonContent: ''
                    } );
                } else {
                    ...
                    deliveryPoint.properties.set( {
                        ...
                        balloonContent: BadZoneContent,
                        balloonContentHeader: 'Адрес вне зоны доставки',
                ...
                setMetka(obj, { notAvaible, zoneId: polygon.properties.get( 'zoneId' ), price: polygon.properties.get( 'price' )});

                function setMetka( obj, props ) {
                    // Задаем подпись для метки.
                ...
                function setData( obj, props ) {
                    const objAdress = obj.getThoroughfare(),
                        objBuilding = obj.getPremiseNumber();

                    let addressString = [objAdress, objBuilding, obj.getPremise()].join( ' ' );
                    if ( addressString.trim() === '' ) {
                        addressString = obj.getAddressLine();
                    }
                    // Записываем адрес и номер дома
                    changeDeliveryData({
                            address: objAdress,
                            building: objBuilding,
                            notAvaible: props.notAvaible,
                            price: props.price,
                            zoneId: props.zoneId
                        });
                    ...
                    deliveryPoint.properties.set( {
                        ...
                        balloonContentHeader: props.price && `${props.price}руб.` || ''
                    } );
                }
    ...
    useEffect( () => {
        if ( window.ymaps ) {
            ...
            // Вставляем скрипт YANDEX MAP в HEAD (КОСТЫЛЬ)
            const script = document.createElement( "script" );
            script.src = "https://api-maps.yandex.ru/2.1?apikey=9bee62c2-b011-46d7-ba9c-12d7fbce3f2a&lang=ru_RU";
            script.async = true;
            document.head.appendChild( script );
        }

        // Проверка на загрузку скрипта
        const YamMapInterval = setInterval( () => {
            if ( !!window.ymaps ) {
                clearInterval( YamMapInterval );
                setLoaded( true );
                window.ymaps.ready( init );
            }
        }, 100 );
        ...
        return ()=>{
            if (YamMapInterval) clearInterval(YamMapInterval);
        }
    }, [] );
    ...
    return <div>
        <TopInfoText>
            Экспресс доставка по Москве в пределах МКАД.
            <p>
                Курьер доставит заказ сразу после изготовления (1-3 рабочих дня)
            </p>
            <p style={{color: COLORS.TEXT_DANGER}}>
                В связи с большим количеством заказов, время доставки может быть увеличено на 1-2 дня.
            </p>
        </TopInfoText>

        <Row>
            <Col w={isMobile ? 1 : 8} x={3}>
                <Input name={'address'}
                       ...
                       id={'suggest'}
                       error={errors && errors.error_address && errors.error_address.text}
                />
            </Col>
            <Col w={isMobile ? 2 : 8}>
                <Input name={'building'}
                       ...
                       onChange={onChangeAddress}
                       // placeholder={'Дом'}
                       error={errors && errors.error_building && errors.error_building.text}
                />
            </Col>
            <Col w={isMobile ? 2 : 8}>
                <Input name={'pd'}
            ...
            <Col w={isMobile ? 2 : 8}>
                <Input name={'flat'}
                       value={addressObj.flat || ''}
                       label={'Квартира*'}
                       onChange={onChangeAddress}
                       error={errors && errors.error_flat && errors.error_flat.text}
        ...
        <AddressBlock adr={addressObj} goToOtherDelivery={() => changeDeliveryData({type: MY_DELIVERY_DDELIVERY})}/>

    </div>;
};

export default U4UDelivery;